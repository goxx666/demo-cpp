...
- <img width="549" height="242" alt="image" src="https://github.com/user-attachments/assets/e9b3d49f-1fbc-4070-9418-1d13397e6471" />
...
- 引用-变量的一个别名
  ...
  - int& b = a；
  - **注意**
    - 引用不是指针，引用不可修改指向，指针可以修改指向
    - 引用创建后必须要初始化，且不能为null
    - 操作使用等于引用变量本身
   
  - 使用
    - 引用传递 void add（int &a，int &b）；，引用对象复制，a、b的值实际操作，不对内存操作
    - 对比：int a传递，数据传递，不影响实际值；
    - 对比：int *a传递，地址传递，影响实际值，直接对内存操作
...
## 动态内存分配
- 申请-申请并分配内存空间，提供指向空间的指针
  - new 类型       申请普通变量空间
  - new 类型[n]    申请数组空间
- 释放-释放new申请的的内存，需要编程处理，c++不会自动处理，可用与处理函数返回（函数结束，C++会自动清空内存）
  - delete 指针     删除普通变量空间
  - delete[] 指针   删除数组空间
  -  
  - int *p = new int;
...
- 数组元素插入、移除
  - new申请新内存空间，复制数据到新数组，delete删除旧数组空间占用，旧数组指针指向新数组地址
  - 指针悬挂-指针指向区域被回收，指针不能使用
...
## 多文件函数编程
- 函数声明写在.h文件中，函数实现写在同名的.cpp文件中，引用.h文件
- include一般导入.h的声明文件，要使用声明文件对应的实现cpp文件，需要在编译同时编译该cpp文件
